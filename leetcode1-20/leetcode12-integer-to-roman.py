...
class Solution:
    def intToRoman(self, num):
        ...
        i = 0
        while num > 0:
            alabo_value = alabo_values[i]
            rome_char = rome_chars[i]
            b = int(num / alabo_value)
            if b == 9:
                rome_value += (rome_char+rome_chars[i-2])
            elif b == 4:
                rome_value += (rome_char+rome_chars[i-1])
            elif b < 4:
                for j  in range(b):
                    rome_value += rome_char
            elif b > 4:
                rome_value += rome_chars[i-1]
                for j in range(5, b):
                    rome_value += rome_char
            num = num % alabo_value
            i+=2
